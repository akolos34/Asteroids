...
public class Ship {
	...
	public void move(int fieldWidth, int fieldHeight) {
		...
		// Keep angle within interval of (0, 2PI), for simplicity
		// also - in theory, a user could overload the capacity of
		// the double type if they kept spinning around
		if (angle > (2 * Math.PI))	
			angle -= 2 * Math.PI;
		else if (angle < 0)
			angle += 2 * Math.PI;
	...
	public Missle fire() {
		...
		// TODO: this time value (40) can be adjusted
